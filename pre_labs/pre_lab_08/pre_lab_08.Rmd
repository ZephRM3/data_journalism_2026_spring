...
The top section of code is pulling just the number out of the adherents percentage column. The second is creating a new data fraom based on number of congregations. 
...
A full bar chart now showing the top ten states.
...
The bar chart is now reorded from lowest to highest in terms of congregations. 
...
The chart's coordinates have been flipped to turn the chart. This cleans up the names. The theme and color were also changed. 
...
The theme changed, where the background is now blue, and the grid the bars are on are now white and even more minimal.
...
It has a whole bunch of other things that make up a chart now, like a title, labels for the axes, and even a caption citing the source on the bottom.
...
A new dataframe with data pertaining to the catholic parishes and priests.
...
Some points were added at each point in the line to the chart by the geom_point aes.
...
All the same aes we did with the bar chart are now added. It has a title, labeled axes, and source caption. 
...
There is now a scatterplot comparing diversity index value to adherents as a percentage of the population. There seems to ba general upward trend which becomes less tightly linear and upward as the adherents percentage increases. There are a couple outliers as well.
...
All of the same aes from the last two charts have been added, and a trend line or line of best fit has also been added as well to give more context to the data points. 
