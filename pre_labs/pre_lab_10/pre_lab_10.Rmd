...
Some of the same columns as a regular file with zip codes and town names, but also columns related to shape like shape length and area, and a geometry column.
...
A map of Maryland that shows the shapes of all of all 526 zipcodes in the state. 
...
It downloaded the package Tigris. 
...
It gave a dataframe of every county in the U.S. with a geometry column. 
...
It created a dataframe of all the MD counties with a geometry column.
...
This is a map of all the counties in Maryland. It does not include the Chesapeake Bay which is a big problem. 
...
It creates a map where the zip code map is overalyed ontop of the county map. 
...
It's a dataframe of PG County's 64 zipcodes with the necessary geometry to make a map. The dataframe was joined on the geometries. 
...
It creates a dataframe counting the churches by zip, and a data frame that has the necessary geometries to make a choropleth map.
...
The rural counties on the fringes have less churches. The more urban zipcodes in population centers have more churches.
...
The libraries were downloaded, including tidycensus. 
...
The code made two data frames. md_county_religion pulls data from the Maryland religion data frame we've been using, and has FIPS codes and religion adherence data as well as most popular congregation. The county populations data frame pulls from the acs suing tidycensus and give the population of every county in Maryland. 
...
This code added a geometry data column for every county in the county population dataframe which will be essential for our map.
...
This code displays the issue in the names of the countyies described above.
...
This code cleaned up county population to make it match the county religion dataframe.
...
This code created a new dataframe that combined the religion data to the county population data.
...
This code added a new column to the dataframe that gives the percentage of congregations per 10,000 people in PG County.
...
You get a map of all the counties in Maryland shaped in by the rate of churches per 10,000 people. The map also labels the counties by the county name though they are kind of messy.
...
This makes the color grading of the map much cleaner and easier to read like in the code earlier in the project. 
...
This code changes the log scale to make it match the rest of the map. In terms of patterns, here the map is kind of reversed from what we saw in the pg county map. The more rural counties have more churches per 10,000 people by far. 
