...
The warning is a parsing error. When problem() gets pasted into the console we get that the problem is coming from column 4 not reading as a number, when the code is expecting to see a number, and thus deletes the data in that line.
...
It has removed the parsing errors because this code is reading much further into the data to determine what the column should be classified as. 
...
The date column is a character.
...
560 zipcodes don't have 5 digits. This is most likely do to an issue with excel dropping a leading 0, or some other issue that occured in the data collection of hte zipcode.
...
There were 309 total payments. However, not all 309 are actually churches, but various different entities that may or may not be related to a church. The amount of payments made is in the hundreds and the, total amount would be well into the millions of dollars.
...
There are some undefined results, and there are issues again with uniformity. For instance, not every year has data for both grants and loans, and sometimes gives strange results like "L" or "contract" which are not consistent.
...
There are several outliers in terms of loan amount like $60 and $95. This doesn't really make sense, and should be considered an outlier.
...
It's made them all lowercase, and removed the spaces as well, and made them into underscores. 
...
The first line is establishing the new dataset based on md_grants_loans. The second is cleaning the name of the column, and the thirs is renaming the grantor column to "source," and the fourth line changes the case of the entire columns in source and grantee to uppercase.
...
There are 58 poteintal duplicates. This does not mean that they are really duplicates and could mean that the state paid that organization twice.
...
There 21197 rows in the new dataframe.
...
This removes the extra numbers in zip+4's that we don't need, and leaves the good zips in the new column zip5.
...
The grantee who received the most money from the state was the BOARD OF CHILD CARE OF THE UNITED METHODIST CHURCH, INC., zipcode 21244. I'm not entirely sure what would make one of these churches more interesting than the other, they all seem to be pretty standard churches, or churches operating education programs. Number 34, The Life Changing Church, is a very interesting name and I've never heard of them before.


```{r}
highest_earning_churches <- cleaned_md_grants_loans |>
