...
**Task** Run the following code to limited to independent churches:
...
The sixth row shows the Chi Alpha University of Maryland church (whatever that is), and also provides that church's PO box adress.
...
```{r}
md_churches |>
  filter(str_detect(NAME, "ADVENTIST"))
```

**Answer**

I picked Adventist because that is the denomination I have been assigned to find for the data collection project, and I thought that it would be interesting for those purposes.
...
The transaction date column is now in the correct format to code dates in, and it is no longer a character but a "date."
...
The month column is all the days of that specific month and year collapsed onto one day, which in effect makes it so we can read that date as being all the data for that unique month.
...
It's finding the average number of parishioners per parish over the given time. That number has steadily been rising sine 1965.
...
This code reordered the dataset around the catholics_per_parish variable, ordering the data from highest average catholics per parish to lowest, which essentially turned the data upside down from what it was in the previous task. 
...
The percentage of parishes without a resident priest has steadily risen in the US since 1965.
...
str_to_title: converts to title case (first letter of each word is capitalized).
...
It's essentially searching the column payee_upper which is where the church name is and assigning a denomination based on the key word found in the name (Baptist, Methodist, Jewish, etc...). If there is no find able keyword it just assigns it NA. This helps us create a more user freindly column in what is kind of confusing data.
...
I would say based on the data yes. In this data set, there appears to be more Baptist denominations than any other denomination, so that denomination making the most transaction makes sense.
...
1.  What is the most common expenditure category? There seems to a set number of categories that this could be, and I think it would be interesting way to arrange the data. Are the majority of churches being up front about what the expenditure is or are they listing it as "other expenses?" 

2. What churches have been making the most transactions mos recently recently? Has there been any change recently? 

3. Where do Catholic churhces rank in terms of expenditures?
